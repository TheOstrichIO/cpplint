...
"""Utility script to automate style stuff."""
...
_STYLER_MODULES = frozenset([u'sort_includes',])
...
def err(message, *args):
  ...
  sys.stderr.write('ERROR: %s' % (formatted))
  raise RuntimeError(formatted.strip())

def warn(message, *args):
  """Warning message printer."""
  sys.stderr.write('WARNING: ' + stringify(message, *args))

def info(message, *args):
  """Info message printer."""
  if not _QUIET:
    sys.stderr.write('INFO: ' + stringify(message, *args))
...
class HFile(object):
  ...
  def __init__(self, include_str):
    ...
    if not match:
      raise Exception('Not an include line: "%s"' % (include_str))
...
def sort_includes(filename, lines):
    ...
    if line.strip().startswith(u'#include'):
      ...
      if hkey in includes:
        ...
        if repr(hfile) == repr(includes[hkey]):
          ...
          warn('"%s" included more than once (consistently) in "%s:%d": %s',
               hfile.name, filename, lnum+1, hfile)
        else:
          # Occurences inconsistent! it's an error.
          err('"%s" included more than once (inconsistently) in "%s:%d": %s',
              hfile.name, filename, lnum+1, hfile)
      else:
        ...
        if is_project_file(hfile.name):
          if hfile.is_system:
            warn('"%s" looks like a project-file, but is included with <> '
                 'in "%s:%d": %s', hfile.name, filename, lnum+1, repr(hfile))
        else:
          if not hfile.is_system:
            warn('"%s" looks like a system-file, but is included with "" '
                 'in "%s:%d": %s', hfile.name, filename, lnum+1, repr(hfile))
    else:
      if in_batch:
  ...
  if includes_batches > 1:
    warn('More than 1 batch of #include\'s in "%s"', filename)
  return new_lines
...
def stylify_file(args, filepath):
  ...
  if new_content != old_content:
    if args.show_diff:
      info('Modified content of %s. Diff:', filename)
    ...
    if '-' == filepath:
      ...
      info('Writing changes back to filepath %s ...', filepath)
      with codecs.open(filepath, 'w', 'utf8', 'replace') as f_out:
        f_out.write(new_content)
  else:
    info('No changes for %s ...', filename)

def stylify(args, files):
  """Stylify files."""
  if args.modules:
    args.modules = set(args.modules)
    assert args.modules <= _STYLER_MODULES
  else:
    args.modules = _STYLER_MODULES
  if files:
    for filepath in files:
      filepath = os.path.normpath(filepath)
      info('Stylifying file %s ...', filepath)
      try:
        stylify_file(args, filepath)
      except RuntimeError:
        info('Skipping file %s ...', filepath)
      else:
        info('Done with file %s ...', filepath)
  else:
    stylify_file(args, '-')

def main():
  """Run neater command on input(s)."""
  parser = argparse.ArgumentParser()
  subparsers = parser.add_subparsers()
  styler_parser = subparsers.add_parser('stylify',
                                        help='Stylify C++ source code')
  styler_parser.add_argument('--no_edit', action='store_true',
                             help='Don\'t write stylified code back to '
                             'source file')
  styler_parser.add_argument('--show_diff', action='store_true',
                             help='Print diffs between input file and '
                             'stylified file to STDERR')
  styler_parser.add_argument('--quiet', action='store_true',
                             help='Don\'t print progress '
                             '(only warnings and errors)')
  styler_parser.add_argument('-m', '--modules', action='append')
  styler_parser.add_argument('--filename',
                             help='Filename in case of STDIN')
